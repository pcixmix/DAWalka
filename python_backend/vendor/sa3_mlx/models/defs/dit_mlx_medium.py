...
class DiT(nn.Module):
    ...
    def __call__(self, x, t, cross_attn_cond_raw, global_cond_raw, local_add_cond=None):
        ...
        """
        B = x.shape[0]
        T = x.shape[2]  # actual time axis length at inference (see dit_mlx.py)
        ...
        if local_add_cond is None:
            ...
            # this server (the T-axis is runtime-dependent).  See the
            # matching fix in dit_mlx.py for context.
