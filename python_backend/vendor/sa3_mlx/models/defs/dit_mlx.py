...
class DiT(nn.Module):
    ...
    def __call__(self, x, t, cross_attn_cond_raw, global_cond_raw, local_add_cond=None):
        ...
        """
        B = x.shape[0]
        T = x.shape[2]  # actual time axis length at inference (may differ from self.T_lat)
        ...
        if local_add_cond is None:
            # Build zeros matching the ACTUAL time axis of x, not the
            # model init-time T_lat.  The DiT is loaded with T_lat=1 in
            # this server (the T-axis is runtime-dependent: an 8 s clip
            # at 44.1 kHz / 4096 samples-per-latent has T_lat≈86, a
            # 47 s clip ≈512).  Broadcasting from the init-time zeros
            # to self.T_lat causes a shape mismatch with x_pp.
            local = mx.zeros((B, T, LOCAL_ADD_COND_DIM))
        else:
